...
echo "2. If you have existing state, run: terraform init -migrate-state -backend-config=config/backend-dev.hcl"
echo "3. If this is first time, run: terraform init -backend-config=config/backend-dev.hcl"
echo "4. Then: terraform plan -var=\"environment=dev\" && terraform apply -var=\"environment=dev\""
