# Documentation

This directory contains all documentation for Mostage Studio project.

## Getting Started

- [Project Structure](structure.md) - Architecture and development guide
- [Infrastructure Setup](infrastructure.md) - AWS Terraform infrastructure setup and deployment
- [Authentication Setup](authentication.md) - AWS Cognito authentication setup guide

## Development

- [CI/CD](ci-cd.md) - Continuous Integration and Deployment setup
- [State Management](state-management.md) - State management patterns and architecture
- [Analytics](analytics.md) - Analytics implementation and tracking
- [Onboarding Tour](onboarding-tour.md) - User onboarding tour system
- [Toolbar](toolbar.md) - Markdown toolbar implementation

## Reference

- [FAQ](faq.md) - Frequently asked questions and explanations
